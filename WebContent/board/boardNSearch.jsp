				...
				<article class="mainmenu section SECTION">
					...
					<%
						if(count != 0){
							...
							%><a href="./BoardNoticeSearch.no?pageNum=<%=startPage-1 %>&search=<%=search %>">[이전]</a>&nbsp;<%
							}
	
							//1~10	11~20	21~30
							for(int i=startPage; i<=endPage; i++){
								%>&nbsp;<a href="./BoardNoticeSearch.no?pageNum=<%=i %>&search=<%=search %>"><%=i %></a>&nbsp;<%
							}
	
							//다음
							if(endPage < pageCount){
								%>&nbsp;<a href="./BoardNoticeSearch.no?pageNum=<%=startPage+pageBlock %>&search=<%=search %>">[다음]</a><%
							}
						}
					%>
