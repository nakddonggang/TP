		...
		<div class="container">
						...
						<div class="adms" >
							...
							<div id="adm_select_box3">
									<select name="sort" id="book_sort">
											...
											<option value="./BookSort.bk?sort=book_subject" <%if(sort.equals("book_subject")) {%>selected<%}%>>제목순</option>
											<option value="./BookSort.bk?sort=book_author" <%if(sort.equals("book_author")) {%>selected<%}%>>저자순</option>
											<option value="./BookSort.bk?sort=book_number" <%if(sort.equals("book_number")) {%>selected<%}%>>인기순</option>
											<option value="./BookSort.bk?sort=book_pubDate" <%if(sort.equals("book_pubDate")) {%>selected<%}%>>신작순</option>
											<option value="./BookSort.bk?sort=book_date" <%if(sort.equals("book_date")) {%>selected<%}%>>입고순</option>
									</select>
							</div>
			...
			<!-- ★갤러리로 보여지는 통합검색 -->
					<ul class="brd_txt_lst" id="book_pic_div">
						...
						} else { for (BookDTO booksortLists  : booksortList){ %>
						<%SimpleDateFormat date = new SimpleDateFormat("yyyy-MM-dd"); %>
						<div class="con_lst">
							<ul class="no_scroll" onclick="location.href='./BookInfo.bk?book_number=<%=booksortLists.getBook_number()%>'">
								<li id="book_pic_hv"><img src="./upload/<%=booksortLists.getBook_file()%>">
									<dl class="book_info_layer">
										<dt><span><%=booksortLists.getBook_subject()%></span></dt>
										<dd>
										<dl class=""> 
											<dt>저자</dt><dd><%=booksortLists.getBook_author()%></dd>                                                                   			        
											<dt>출판사</dt><dd><%=booksortLists.getBook_publisher()%></dd>                                                          			        
											<dt>출판년도</dt><dd><%=date.format(booksortLists.getBook_pubDate())%></dd>                                                        			        
											<dt>반납상태</dt><dd><%=booksortLists.getBbook_bstate()%></dd>																   			        
										</dl>                                                                                      		   
										</dd>                                                                                          		   
									</dl>
					...
					<!-- ★게시판으로 보여지는 통합검색 -->
						...
						<div class="con_lst">
							<ul class="no_scroll" onclick="location.href='./BookInfo.bk?book_number=<%=booksortLists.getBook_number()%>'">
								...
								<li class="adm_col_type" id="adm_book_high"><img src="./upload/<%=booksortLists.getBook_file()%>" width="70px" height="80px"></li>
				...
				if (count != 0) {
					...
					if (startPage > pageBlock) {
						%><a href="./BookSort.bk?sort=<%=sort%>&pageNum=<%=startPage - pageBlock%>">[이전]</a><%
					}
					// 1~10		11~20		21~30
					for (int i = startPage; i <= endPage; i++) {%>
					<a href="./BookSort.bk?sort=<%=sort%>&pageNum=<%=startPage - pageBlock%>">[<%=i%>]</a><%		
					}
					// 다음 // if (endPage와 pageCount를 비교)
					if (endPage<pageCount){%>
						<a href="./BookSort.bk??sort=<%=sort%>&pageNum=<%=startPage - pageBlock%>">[다음]</a><%
					}
				} // if count 괄호 %>		
				</div>	
