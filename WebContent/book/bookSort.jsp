 ...
 %>
									<div class="con_lst">
										<ul class="no_scroll">
											...
											<%if (booksortLists.getBbook_bstate()=="0"){ %> 대출가능 <% }
											else { 
												if (booksortLists.getBbook_bdate()!=null){%>
												<%=date.format(booksortLists.getBbook_bdate())%>~<%}
												else{%> 대출불가<%}%> 
											<%}%>
											</li>
											<%if(member_id!=null){ %>
											<li class="adm_col_type"  id="adm_book_high">
												<%if (booksortLists.getBbook_bstate()=="1"){ %>
													<input type="button" rel="<%=booksortLists.getBook_number()%>" class="bbutton" value="대출신청">
												<%} else {%>
												<input type="button" rel="<%=booksortLists.getBook_number()%>" class="rbutton" value="대출예약">
											<%}%>
											</li><%}%>
										</ul>
									</div>
