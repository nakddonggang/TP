 ...
 %>
				...
				<!-- 버튼 css 부분 -->	
								...
								<div class="paginate">
									...
									if(startPage > pageBlock)	{ %><a href="./BookSearchPic.bk?pageNum=<%=startPage - pageBlock%>" class="prev"><span class="hide">이전 페이지</span></a><%	}
									for (int p = startPage; p <= endPage; p++) {	
									if(p==Integer.parseInt(pageNum)) {%> &nbsp;<strong id="currentPage" title="현재 페이지"><%=p %></strong> &nbsp;<%}
									else {%> &nbsp;<a href=./BookSearchPic.bk?pageNum=<%=p%>><%=p %></a> &nbsp;<%}
									}
									if(endPage < pageCount){	%><a href="./BookSearchPic.bk?pageNum=<%=startPage+pageBlock%>" class="next"><span class="hide">다음 페이지</span></a><% }
									%>
						 		</div>
