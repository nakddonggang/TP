	...
	function onload(pageNum, sort){
		...
		$.ajax({
			...
			success:function(result){
				...
				// ajax 변수 
				var adm_ajax = JSON.parse("["+result+"]");
				var count=adm_ajax[adm_ajax.length-7].count;
				var sort=adm_ajax[adm_ajax.length-6].sort;
				pageNum=adm_ajax[adm_ajax.length-5].pageNum;
				var pageCount=adm_ajax[adm_ajax.length-4].pageCount;
				var pageBlock=adm_ajax[adm_ajax.length-3].pageBlock;
				var startPage=adm_ajax[adm_ajax.length-2].startPage;
				var endPage=adm_ajax[adm_ajax.length-1].endPage;
				...
				var member_id="${member_id}";
					...
					// content 내용 넣기
							...
							if(count ==0 ){
								...
								for(var i=0; i<adm_ajax.length-7; i++){
									...
									+adm_ajax[i].book_number+"\"\'>"
									+adm_ajax[i].book_number+"</li><li class='adm_pic' id='adm_book_high' onclick='location.href=\"./AdminBookInfo.am?book_number="
									+adm_ajax[i].book_number+"\"\'><img src='./upload/book/"
									+adm_ajax[i].book_file+"' width='70px' height='80px'></li><li class='adm_subs2' id='adm_book_high'onclick='location.href=\"./AdminBookInfo.am?book_number="
									+adm_ajax[i].book_number+"\"\'>"
									+adm_ajax[i].book_subject+"</li><li class='adm_name over_dot' id='adm_book_high' onclick='location.href=\"./AdminBookInfo.am?book_number="
									+adm_ajax[i].book_number+"\"\'>"
									+adm_ajax[i].book_author+"</li><li class='adm_ten over_dot' id='adm_book_high' onclick='location.href=\"./AdminBookInfo.am?book_number="
									+adm_ajax[i].book_number+"\"\'>"
									+adm_ajax[i].book_publisher+"</li><li class='adm_ten'><input type='button' id='"
									+i+"' value='보기' class ='info_bbutton2' ></li></ul>";
										$('#text3_ap').append(text4);
										
										if (adm_ajax[i].bbook_bstate==0) {
...
//  								adm_ajax[i].bbook_bdate+"~"+adm_ajax[i].bbook_rdate+"대출중";
// 								$('.mod_dv'+i).append(mod1);
// 							}
...
// 							if (adm_ajax[i].dbook_state==0){
// 								var mod5="<li><ul class='row_sub'><li class='title'>책상태</li><li class='inp_form'>상태양호";
// 								$('.mod_dv'+i).append(mod5);
// 							} else {
// 								var mod5="<li><ul class='row_sub'><li class='title'>책상태</li><li class='inp_form'>"+adm_ajax[i].dbook_state+"";
// 								$('.mod_dv'+i).append(mod5);
// 							}
